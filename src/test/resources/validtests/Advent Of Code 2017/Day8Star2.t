...
class Condition =
	...
	Def new(register: String, amount: Int, op: String) =
		this.register = register
		this.amount = amount
		this.op = op

	Def Applies(registers: HashMap<String, Int>): Bool =
		val a = registers[register]
		val b = amount
	...
	Def toString() = register + " " + op + " " + amount
...
class Instruction =
	...
	Def new(register: String, amount: Int, condition: Condition) =
		this.register  = register
		this.condition = condition
		this.amount    = amount

	Def Execute(registers: HashMap<String, Int>) =
		if(!condition.Applies(registers))
			return -1
		val newAmount = registers[register] + amount
		registers[register] = newAmount
		return newAmount

	Def toString() = register + " " + amount + " " + condition


def parse(registers: HashMap<String, Int>, line: String) =
	val s = line.Split("\\s+")
	val reg = s[0]
	registers.Add(reg, 0)

	val incOrDec = s[1]
	val amount = s[2].ToInt() * (incOrDec == "inc" ? 1 : -1)
	val conditionReg = s[4]
	val operator = s[5]
	val num = s[6]

	val condition = new Condition(conditionReg, num.ToInt(), operator)

	new Instruction(reg, amount, condition)

val lines = input.Split("\r?\n")
val registers = new HashMap<String, Int>()
val instructions = new Instruction[lines.Size()]
for(var i = 0; i < lines.Size(); i++)
	instructions[i] = parse(registers, lines[i])

var max = 0
for(val instruction in instructions)
	max = Math.max(max, instruction.Execute(registers))

println("Max ever: " + max) // res: Max ever: 6369