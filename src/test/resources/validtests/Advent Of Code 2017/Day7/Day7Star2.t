...
rtefbvp (45)
wljtjp (6)
sdxbol (2578) -> qwsjrs, dnqva, ddssmn
yckzzdv (37)
ncyhsb (76)
sxoxuhy (275)
whiaid (74)
...
class Program =
	...
	def parseWeight(weight: String) = weight.substring(1, weight.Size() - 1).ToInt()


	def parseProgram(prog: String) =
		val split = prog.Split(" ")
		this.name = split[0]
		this.weight = parseWeight(split[1])


	Def FillParent(programs: Program[]) =
		if(childString == "")
			this.children = new Program[0]
			return

		val childStrings = childString.split(", ")
		this.children = new Program[childStrings.Size()]
		for(var i = 0; i < childStrings.Size(); i++)
			val c = childStrings[i].Trim()
			var child: Program? = null
	...
	Def new(s: String) =
		...
		this.childString = split.Size() > 1 ? split[1] : ""


	Def toString() = "Program(" + this.name + ", " + weight + ")"


val lines = input.split("\r?\n")

val programs = new Program[lines.Size()]
for(var i = 0; i < lines.Size(); i++)
	programs[i] = new Program(lines[i])

for(val program in programs)
	program.FillParent(programs)

var topMostProgram: Program? = null
for(val program in programs)
	if(program.parent == null)
		topMostProgram = program
		break

def calculateWeights(program: Program, d: Int): Int =
	var w = program.weight
	val children = program.children
	val weights = new Int[children.Size()]
	var seenWeight = -1
	var unbalanced = false
	for(var i = 0; i < children.Size(); i++)
		val weight = calculateWeights(children[i], d + 1)
		if(seenWeight == -1)
			seenWeight = weight
		else if(seenWeight != weight)
			unbalanced = true
	...
	if(unbalanced)
		println(d + ": " + program.name + "(" + program.weight + "): " + Arrays.toString(weights))
		for(var i = 0; i < children.Size(); i++)
			println("   " + children[i] + ": " + weights[i])

	w

calculateWeights(topMostProgram, 0)
