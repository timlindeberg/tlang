...
class Particle =
	...
	Def Position(t: Long) = pos + (vel * t) + ((1/2) * acc * (t * t))

	Def DistanceToOrigin() = Math.abs(pos.X) + Math.abs(pos.Y) + Math.abs(pos.Z)

	Def toString() = "p=<" + pos + ">, v=<" + vel + ">, a=<" + acc + ">"


Def ClosestParticle(particles: Particle[]) =
	var minDistance = Long.MaxValue()
	var closestParticle = - 1

	for(var i = 0; i < particles.Size(); i++)
		val dist = particles[i].DistanceToOrigin()
		if(dist < minDistance)
			minDistance = dist
			closestParticle = i
...
Def ParseParticles(input: String) =
	val n = "(\\-?\\d+)"
	val position = "<" + n + "," + n + "," + n + ">"
	val regex = Pattern.compile("p=" + position + ", v=" + position + ", a=" + position)

	val lines = input.Split("\r?\n")
	val particles = new Vector<Particle>()
...
for(var iterations = 0; iterations < 100; iterations++)
	...
	for(var i = 0; i < particles.Size(); i++)
		val particlesToRemove = new Vector<Particle>()
		for(var j = 0; j < particles.Size(); j++)
			if(i != j && particles[i].pos == particles[j].pos)
				particlesToRemove.Add(particles[i])
				particlesToRemove.Add(particles[j])
		for(val toRemove in particlesToRemove)
			particles.Remove(toRemove)


println(particles.Size()) // res: 461
