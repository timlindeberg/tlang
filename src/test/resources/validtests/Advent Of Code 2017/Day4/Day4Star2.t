...
class AnagramString =
	...
	val chars: HashMap<Char, Char> = new HashMap<Char, Char>()

	Def new(s: String) =
		for(val c in s)
			chars.Add(c, c)

	Def ==(a: AnagramString, b: AnagramString) = a.chars == b.chars
	Def #(s: AnagramString) = #s.chars


var invalid = 0
val passPhrases = input.split("\r?\n")

for(val passPhrase in passPhrases)
	val set = new HashMap<AnagramString, AnagramString>()
	val words = passPhrase.split("\\s+")
	for(val word in words)
		val a = new AnagramString(word)
		set.Add(a, a)
	if(set.Size() != words.Size())
		invalid += 1

println("Valid: " + (passPhrases.Size() - invalid)) // res: Valid: 265
