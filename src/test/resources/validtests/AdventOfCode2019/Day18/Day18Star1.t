...
class Pos =
	...
	Def new(pos: Vec2<Int>, keys: Long) =
		Pos = pos
		Keys = keys

	Def #(pos: Pos): Int = 31 * #pos.Pos + #pos.Keys
	Def ==(a: Pos, b: Pos) = a.Pos == b.Pos && a.Keys == b.Keys

	Def toString() = "(" + Pos + ", " + Keys + ")"

class Day18 : Lol =

	val graph: Graph<Edge<Int>>
	val board: String[]
	val Width: Int
	val Height: Int
	val Visited = new HashSet<Pos>()
	val Parents = new HashMap<Pos, Pos>()

	val Right = new Vec2<Int>(0, 1)
	val Left = new Vec2<Int>(0, -1)
	val Up = new Vec2<Int>(-1, 0)
	val Down = new Vec2<Int>(1, 0)
	...
	Def new(input: String) =
		...
		Width = board[0].Size()
		AllKeys = GetAllKeys()
		graph = new Graph<Edge<Int>>(25)

	Def Run() =
		PrintBoard()
		val start = FindStart()
		println(AllKeys.ToBinaryString())
		println("start: " + start)
		Traverse(start, 0)

		val path = MakePath(start)
		println(path)

	Def Traverse(pos: Vec2<Int>, keys: Long) =
		Visited.Add(new Pos(pos, keys))

		println("pos: " + pos)

		var newKeys = IsKey(pos) ? AddKey(keys, Board(pos)) : keys

		if(newKeys == AllKeys)
			println("ALLKEYS")
			return

		TryTraverse(pos, pos + Up, newKeys)
		TryTraverse(pos, pos + Down, newKeys)
		TryTraverse(pos, pos + Left, newKeys)
		TryTraverse(pos, pos + Right, newKeys)

	Def TryTraverse(from: Vec2<Int>, to: Vec2<Int>, keys: Long): Unit =
		println("from: " + from)
		println("to: " + to)
		if(!CanTraverse(to, keys))
			return

		Parents[new Pos(to, keys)] = new Pos(from, keys)
		Traverse(to, keys)

	Def CanTraverse(pos: Vec2<Int>, keys: Long) =
		if(Visited.Contains(new Pos(pos, keys)) || !IsInBounds(pos))
			return false
		...
		if(IsDoor(c))
			val neededKey = c.ToLower()
			val check = 0.SetBit(neededKey)
			if (keys[check] == 1)
				return true
	...
	Def IsInBounds(pos: Vec2<Int>) = pos.X >= 0 && pos.X < Width && pos.Y >= 0 && pos.Y < Height

	Def AddKey(keys: Long, key: Char) = keys.SetBit(key - 'a')

	Def Board(pos: Vec2<Int>) = board[pos.Y][pos.X]

	Def IsKey(pos: Vec2<Int>) = IsKey(Board(pos))
	Def IsKey(c: Char) = c >= 'a' && c <= 'z'
	Def IsDoor(c: Char) = c >= 'A' && c <= 'Z'

	Def FindStart(): Vec2<Int> =
		for (var y = 0; y < Height; y++)
			for (var x = 0; x < Width; x++)
				if(board[y][x] == '@')
