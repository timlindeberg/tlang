...
class PointAndDistance =
	Var Point: Point
	Var Distance: Int
	Def new(point: Point, distance: Int) =
		Point = point
		Distance = distance

class Line =
	Var Start: PointAndDistance
	Var End: PointAndDistance

	Def new(start: PointAndDistance, end: PointAndDistance) =
		Start = start
		End = end
...
class Day3 =
	...
	Def Run() =
		...
		for (val l1 in lines1)
			for (val l2 in lines2)
				...
				if (!intersection)
					continue
				val distance = l1.Start.Distance + (Distance(l1.Start.Point, intersection)) +
				               l2.Start.Distance + (Distance(l2.Start.Point, intersection))
				if (distance != 0)
					minDistance = Math.min(minDistance, distance)
		println(minDistance) // res: 27306
	...
	Def CalculateLines(input: String) =
		...
		for(val line in input.Split(","))
			...
			val p1 = new PointAndDistance(pos, distance)
			distance += instruction.Length
			val p2 = new PointAndDistance(newPos, distance)
	...
	Def Intersection(l1: Line, l2: Line): Point? =
		val p0 = l1.Start.Point
		val p1 = l1.End.Point
		val p2 = l2.Start.Point
		val p3 = l2.End.Point
