...
Def GetIds(events: Vector<Event>) =
	val ids = new Vector<Int>()
	for(val event in events)
		if(event.EventType is Begins)
			val begins = event.EventType as Begins
			ids.Add(begins.Id)
...
Def GetSleepiest(events: Vector<Event>) =
	var ids = new HashMap<Int, Int>()
	var times = new HashMap<Int, Int>()
	for(val currentId in GetIds(events))
		val minutes = GetSleepyMinutes(events, currentId)
		for(val m in minutes)
			val t = times.Get(m.Key()) ?: 0
			if(m.Value() >= t)
				times[m.Key()] = m.Value()
				ids[m.Key()] = currentId
