...
val input = `position=<-41214, -10223> velocity=< 4,  1>
...
class Star =
	...
	Def new(pos: Point, velocity: Point) =
		Pos = pos
		Velocity = velocity
...
class Day10 =
	...
	val regex = Pattern.compile(`position=< *(-?\d+), *(-?\d+)> velocity=< *(-?\d+), *(-?\d+)>`)
	val stars = new Vector<Star>()

	val Iterations = 10360
	...
	Def new(input: String) =
		...
		for(val line in lines)
			...
			m.matches()
			val pos = new Point(m.group(1).ToInt(), m.group(2).ToInt())
			val velocity = new Point(m.group(3).ToInt(), m.group(4).ToInt())
			stars.Add(new Star(pos, velocity))

	Def Run() =
		for(var i = 0; i < Iterations; i++)
			val min = new Point(0, 0)
			val max = new Point(0, 0)
			val set = new HashSet<Point>()

			for(val star in stars)
				star.Pos += star.Velocity
				min.X = Math.min(min.X, star.Pos.X)
				min.Y = Math.min(min.Y, star.Pos.Y)

				max.X = Math.max(max.X, star.Pos.X)
				max.Y = Math.max(max.Y, star.Pos.Y)
				set.Add(star.Pos)

			if(IsInteresting(set))
				println(i)
				Print(set, min, max)

	Def IsInteresting(set: HashSet<Point>) =
		var longest = 0
		val visited = new HashSet<Point>()
		for(val star in stars)
			longest = Math.max(longest, Traverse(star.Pos, set, visited))
		longest > 10

	Def Traverse(pos: Point, set: HashSet<Point>, visited: HashSet<Point>) =
		var distance = 0
	...
	Def Print(set: HashSet<Point>, min: Point, max: Point) =
		val set = new HashSet<Point>()
		for(val star in stars)
			set.Add(star.Pos)
		for(var y = min.Y; y <= max.Y; y++)
			for(var x = min.X; x <= max.X; x++)
				if(set[new Point(x, y)]) print("#")
				else                     print(".")
			println()

new Day10(input).Run() // res: HRPHBRKG
