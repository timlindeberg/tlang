...
case object PrintOutputFlag extends ArgumentFlag[Set[String]] {
  ...
  override def description(implicit formatter: Formatter): String =
    s"""
       ...
       |Type ${ CompilerHelpFlag.formattedName } ${ highlight(name) } for more information.
      """

  override def extendedDescription(implicit formatter: Formatter): String =
    s"""
       |The $formattedName flag prints the output after a given compiler phase.
  ...
  override protected def verify(arg: String)(implicit errorContext: ErrorStringContext): Unit = {
    ...
    if (phase notIn compilerPhases) {
      ...
      error(err"$phase is not a valid argument for --$name.${ suggestion }See ${ HelpFlag.formattedName } for more information.")
    }
  }
