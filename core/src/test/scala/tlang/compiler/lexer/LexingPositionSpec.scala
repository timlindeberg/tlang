...
class LexingPositionSpec extends CompilerIntegrationTestSpec {
  ...
  def testPositions(predicate: Token => Boolean, positions: Position*): Unit = {
    ...
    tokens.zip(positions) foreach { case (foundPos, expectedPos) => foundPos shouldBe expectedPos }
  }
  ...
  "Basic Tokens" in {
    testPositions(_.kind.str.length > 0,
      ...
      Position(18, 4, 18, 10), //   object
      Position(18, 11, 18, 16), //  class
