...
import tcompiler.error.{Boxes, Formatting}
...
object Flags {
  ...
  case object PrintOutput extends OptionalArgumentFlag {
    ...
    override val defaultArg: String = Desugaring.stageName
    ...
    override def description(formatting: Formatting): String = {
      import formatting.colors._
      s"""
         |Prints the output after a given compiler stage.
    ...
    override def extendedDescription(formatting: Formatting): String = {
      ...
      val stages = Main.CompilerStages.map(stage => "  " + Blue(stage.stageName.capitalize)).mkString("\n")
      s"""|The --${Magenta(flag)} flag prints the output after a given compiler stage.
          |This can be the tokens produced by the Lexer, a pretty printed AST or the byte code generated by the Codegeneration stage.
          |The compiler stages are executed in the following order:
          |
          |$stages
          |
  ...
  case object Formatting extends ArgumentFlag {
    ...
    override def extendedDescription(formatting: Formatting): String = {
      ...
      val desc =
        s"""|The --${Magenta(flag)} flag determines what style to use for all output produced by the compiler.
            |The style '${Blue("Simple")}' only produces ASCII-characters and no colors which can be useful when the compiler is ran on simpler terminals.
